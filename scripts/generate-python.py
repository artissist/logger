...
def generate_type_alias(shape_name: str, shape: Dict[str, Any]) -> str:
    ...
    if shape['type'] == 'list':
        ...
        key_type = to_python_type(shape.get('key', {}).get('target', 'smithy.api#String'))
        value_type = to_python_type(shape.get('value', {}).get('target', 'smithy.api#String'))
        # Use string annotation for forward references
...
'''
    ...
    basic_types = ['ServiceName', 'Environment', 'LogId', 'UserId', 'SessionId', 'RequestId', 'Timestamp']
    ...
    def generate_emoji_mappings():
        ...
        emoji_mappings += 'DEFAULT_EMOJI_MAPPINGS: Dict[str, Dict[str, Union[str, bool]]] = {\n'
        ...
        for event, config in mappings.items():
            ...
            emoji_mappings += f'        "is_default": {str(config["isDefault"]).title()}\n'
            emoji_mappings += '    },\n'
        ...
        emoji_mappings += 'TYPED_EMOJI_MAPPINGS: Dict[LogEvent, Dict[str, Union[str, bool]]] = {\n'
    ...
    print(f'📊 Generated {len([name for name in shapes.keys() if name.startswith("artissist.logging#")])} types')
    if emoji_mappings:
        print('🎨 Generated standardized emoji mappings')
