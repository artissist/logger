...
class ConsoleAdapter(LogAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.output_stream = sys.stderr if config.get("use_stderr", False) else sys.stdout
    ...
    async def write(self, message: LogMessage, formatted_message: str):
        ...
        if message.error:
            ...
            if self.use_colors and sys.stdout.isatty():
                error_output = f"{self.COLORS[LogLevel.ERROR]}{error_output}{self.RESET}"
        ...
        if message.metrics:
            ...
            if metrics_dict:
                metrics_output = "  METRICS: " + ", ".join(f"{k}={v}" for k, v in metrics_dict.items())
                if self.use_colors and sys.stdout.isatty():
