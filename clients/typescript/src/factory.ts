...
export class LoggerFactory {
     ...
     */
    static createFrontendLogger(config: Omit<LoggerConfig, 'adapters'> & { 
     ...
     */
    static createBackendLogger(config: Omit<LoggerConfig, 'adapters'> & { 
     ...
     */
    static createAgentLogger(config: Omit<LoggerConfig, 'service'> & { 
        agentId: string;
        agentType?: string;
    }): Logger {
     ...
     */
    static createInfrastructureLogger(config: Omit<LoggerConfig, 'service'> & {
        stackName?: string;
        deploymentId?: string;
    }): Logger {
    ...
    static createFromEnvironment(config: Partial<LoggerConfig> = {}): Logger {
        ...
        const envConfig: LoggerConfig = {
            service: process.env.SERVICE_NAME || LoggerFactory.defaultConfig.defaultService!,
            environment: process.env.NODE_ENV || LoggerFactory.defaultConfig.defaultEnvironment!,
    ...
    private static resolveConfig(config: LoggerConfig): Required<LoggerConfig> {
        return {
            service: config.service || LoggerFactory.defaultConfig.defaultService!,
            environment: config.environment || LoggerFactory.defaultConfig.defaultEnvironment!,
            emojis: config.emojis ?? LoggerFactory.defaultConfig.defaultEmojis!,
            adapters: config.adapters || LoggerFactory.defaultConfig.defaultAdapters!,
            level: config.level || LoggerFactory.defaultConfig.defaultLevel!,
