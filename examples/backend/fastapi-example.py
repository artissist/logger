...
class MockMosaicLogger:
    ...
    def __init__(
        self, service: str, environment: str, emojis: bool = False, context: dict = None
    ):
    ...
    def _log(self, level: str, message: str, **kwargs):
        ...
        print(
            f"[{level}] [{self.service}] {prefix}{message} | correlation_id={correlation_id}"
        )
...
async def health_check(request_logger: MockMosaicLogger = Depends(get_logger)):
    request_logger.debug(
        "Health check requested", event="API_REQUEST", metadata={"endpoint": "/health"}
    )
...
async def create_project(
    project_data: ProjectCreate, request_logger: MockMosaicLogger = Depends(get_logger)
):
    start_time = time.time()
...
async def get_project(
    project_id: str, request_logger: MockMosaicLogger = Depends(get_logger)
):
    request_logger.debug(
...
async def login(
    credentials: dict, request_logger: MockMosaicLogger = Depends(get_logger)
):
    start_time = time.time()
    ...
    try:
        ...
        request_logger.error(
            ...
            event="ERROR_OCCURRED",
            error={"type": type(e).__name__, "message": str(e), "operation": "login"},
            metrics={"duration_ms": round((time.time() - start_time) * 1000, 2)},
        )
